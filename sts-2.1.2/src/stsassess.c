...
void	openOutputStreamsmodified(int option,int numOfBitStreams);
...
main(int argc, char *argv[])
{
	...
	if ( argc != 5 ) {
		printf("Usage: %s <stream length> <filename> <test> <bitstreams>\n", argv[0]);
		//printf("   <stream length> is the length of the individual bit stream(s) to be processed ++ asdasdasdasd\n");
		return 0;
	}
    streamFile = argv[2];
	...
	//chooseTests();
    if (atoi(argv[3]) >=0 && atoi(argv[3]) < NUMOFTESTS + 1)
        testVector[atoi(argv[3])] = 1;
    else{
        printf("Not valid test number");
        exit(-1);
    }
	if ( testVector[0] == 1 ) //all tests
		for( i=1; i<=NUMOFTESTS; i++ )
			testVector[i] = 1;

	//fixParameters(); //not needed, we stay with defaults
	openOutputStreamsmodified(option,atoi(argv[4]));
	...
	readHexDigitsInBinaryFormat(fp); //we only read binary
	fclose(fp);
...
void
openOutputStreamsmodified(int option,int numOfBitStreams)
{
